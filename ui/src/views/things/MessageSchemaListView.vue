...
const sort = ref('namespace,name,-version')
...
const columns: Column<MessageSchema>[] = [
  ...
  { key: 'version', sortable: '-version', label: 'Version', mobileLabel: 'Version' },
      ...
      >
        <template #cell-identity="{ item }">
          <div>
            ...
            <div v-if="item.description" :title="item.description" class="text-sm text-base-content/60 line-clamp-1 mt-1">{{ item.description }}</div>
          </div>
        </template>
